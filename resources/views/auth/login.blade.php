...
@section('title', 'Login')
@section('content')
    <img src="{{ URL::asset('admin/auth/img/github.png') }}" class="user">
    ...
    <form method="POST" action="{{ route('login.custom') }}">
        ...
        <div class="inputbox{{ $errors->has('email') ? ' has-error' : '' }}">
            <input type="email" id="email" name="email" value="{{ old('email') }}" required autofocus placeholder="email anda">
            <span><i class="fa fa-envelope" aria-hidden="true"></i></span>
        </div>
        <div class="inputbox{{ $errors->has('password') ? ' has-error' : '' }}">
            <input id="password" type="password" class="form-control" name="password" required placeholder="password anda">
